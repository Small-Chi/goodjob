      ...
      <template v-slot:default="dialog">
        <v-card color="var(--color-white)">
              ...
              <v-row justify="space-around">
                <v-dialog v-model="dialog2" persistent max-width="600px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn v-bind="attrs" v-on="on" class="textWhite btn1 rounded-pill" style="font-size: 21px;"
                      width="150" height="150" plain>
                      接案
                    </v-btn>
                  </template>
                  <!-- 接案會員登入 -->
          ...
          <div class="d-flex justify-center">
            ...
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="var(--color-white)" dark v-bind="attrs" v-on="on" class="btn2 textBlue rounded-0" plain
                max-height="20">
                成為接案者
              </v-btn>
            </template>
            <v-card>
              <v-toolbar color="var(--color-blue)" style="color: var(--color-white);">
                ...
                <v-spacer></v-spacer>
                <v-toolbar-items>
                  <v-btn class="savebtn" color="var(--color-white)" :ripple="false">
                    Join Us
                  </v-btn>
                </v-toolbar-items>
              </v-toolbar>
              <v-list class="list">
                ...
                <v-card-text>
                  <v-container>
                    <v-row>
                      <v-col cols="12">
                        <v-text-field label="名稱" hint="中文字長度為 1 到 4 個字" :rules='inputRules1'
                          v-model="form.username" :state="state.name"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6">
                        <v-text-field label="帳號" hint="長度為 4 到 20 個字" :rules='inputRules2'
                          v-model="form.account" :state="state.account"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6">
                        <v-text-field label="密碼" type="password" hint="長度為 4 到 20 個字" :rules='inputRules3'
                          v-model="form.password" :state="state.password"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6">
                        <v-text-field label="信箱" hint="長度為 4 到 20 個字" :rules='inputRules4' v-model="form.email"
                          :state="state.email"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6">
                      ...
                      <v-col cols="12" sm="6">
                        <v-select v-model="form.position" :items="['平面設計師', '攝影師', '室內設計師', '插畫家', '創作者', '藝術家', '其他']" label="職業類別">
                        </v-select>
                      </v-col>
                      <v-col cols="12" sm="6">
                        <v-autocomplete v-model="form.technology"
                          ...
                          label="擅長工具" multiple></v-autocomplete>
                      </v-col>
                      <v-col cols="12" sm="6">
                        <v-expansion-panels>
                          <v-expansion-panel>
